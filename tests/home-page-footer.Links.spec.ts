...
test.describe("Home Page Footer Links Tests", () => {
  ...
  test("click upwork footer", async ({ page }) => {
  ...
  test("click twitter footer", async ({ page }) => {
  ...
  test("click linkedin footer", async ({ page }) => {
    const linkedinLinkSelector = ".elementor-repeater-item-eb3cdb2";

    // Wait for the element to be visible
    await page.waitForSelector(linkedinLinkSelector, {
      state: "visible",
      timeout: 300000,
    });
    ...
    await page.screenshot({
      path: "before_click_linkedin_link.png",
      timeout: 300000,
    });
    ...
    await page.screenshot({
      path: "after_click_linkedin_link.png",
      timeout: 300000,
    });
  });

  test("click youtube footer", async ({ page }) => {
