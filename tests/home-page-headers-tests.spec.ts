...
test("Verify Services", async ({ page }) => {
  ...
  await page.waitForSelector(servicesLinkXPath, {
    state: "visible",
    timeout: 60000,
  });
  ...
  await expect(page).toHaveURL(
    "https://softwarequalityassociates.com/services/"
  );
...
test("Verify About Us", async ({ page }) => {
  // Set a higher timeout for the test
  test.setTimeout(120000); // 2 minutes
  ...
  // Wait for the element to be visible
  await page.waitForSelector(aboutUsLinkXPath, {
    state: "visible",
    timeout: 60000,
  });
...
test("Verify STU Link", async ({ page }) => {
  ...
  // Wait for the element to be visible
  await page.waitForSelector(stuLinkXPath, {
    state: "visible",
    timeout: 60000,
  });
...
test("Verify Contact Us", async ({ page }) => {
  ...
  // Wait for the element to be visible
  await page.waitForSelector(contactUsLinkXPath, {
    state: "visible",
    timeout: 60000,
  });
  ...
  await page.screenshot({
    path: "before_click_contact_us.png",
    timeout: 30000,
  });
  ...
  await page.waitForSelector(contactUsSection, {
    state: "visible",
    timeout: 60000,
  });
...
test("Verify Shop Link", async ({ page }) => {
  ...
  // Wait for the element to be visible
  await page.waitForSelector(shopLinkXPath, {
    state: "visible",
    timeout: 60000,
  });
  ...
  await expect(page).toHaveURL(
    "https://www.redbubble.com/people/sqassociates/shop"
  );
